...
for pattern in "tserentserenov" "PACK-MIM" "aist_bot_newarchitecture" "DS-Knowledge-Index-Tseren"; do
    count=$(grep_count "$pattern" "$TEMPLATE_DIR" --include="*.md" --include="*.sh" \
            --include="*.json" --include="*.plist" --include="*.yaml")
    if [ "$count" -gt 0 ]; then
        [ "$CHECK1_FAIL" -eq 0 ] && echo "FAIL"
        echo "  Found '$pattern' in $count locations:"
        grep -rn "$pattern" "$TEMPLATE_DIR" --include="*.md" --include="*.sh" \
            --include="*.json" --include="*.plist" 2>/dev/null | head -3 || true
        CHECK1_FAIL=1
        FAIL=1
    fi
done
...
count=$(grep_count '/Users/' "$TEMPLATE_DIR" --include="*.md" --include="*.sh" \
        --include="*.json" --include="*.plist")
if [ "$count" -gt 0 ]; then
    echo "FAIL ($count hits)"
    grep -rn '/Users/' "$TEMPLATE_DIR" --include="*.md" --include="*.sh" 2>/dev/null | head -3 || true
    FAIL=1
else
    echo "PASS"
fi
